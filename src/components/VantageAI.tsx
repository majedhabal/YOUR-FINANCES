...
export const VantageAI: React.FC<VantageAIProps> = ({
  ...
  const handleExecuteInsightTask = async () => {
    ...
    try {
      const enrichedPrompt = `${userMessage}\n\nSystem Instruction Constraint: You are Vantage AI. You must NEVER give direct recommendations or tips. You must ALWAYS phrase any advice, tips, suggestions, or insights as "Based on research...", "Online sources suggest...", or "According to financial research...". Never state recommendations or tips directly.`;
      const payload = {
              ...
              <div className="flex flex-col">
                ...
                <div className="flex flex-col gap-1 mt-0.5">
                  <span className="text-[10px] text-[#366945] font-normal" style={{ fontFamily: "'Google Sans', sans-serif" }}>
                    Tokens: {typeof profile?.vantageAiTokens === 'number' ? profile.vantageAiTokens.toLocaleString() : '0'}
                  </span>
                  <button
              ...
              <div className="flex flex-col gap-4 w-full">
                  ...
                  <div key={index} className={`flex w-full ${msg.sender === 'user' ? 'justify-end pl-6' : 'justify-start pr-6'} leading-relaxed box-border`}>
                    {msg.sender === 'user' ? (
                      ...
                      <div className="flex gap-3 items-start w-full box-border">
                        ...
                        <div className="p-4 bg-neutral-50 text-neutral-800 font-body-md text-sm rounded-2xl rounded-bl-sm border border-neutral-100 space-y-2 break-words whitespace-pre-wrap flex-1">
                          {msg.text}
                        </div>
                      </div>
                    )}
                ...
                {messages.length === 0 && response && (
                  <div className="flex gap-3 items-start pr-6 leading-relaxed w-full box-border">
                    ...
                    <div className="p-4 bg-neutral-50 text-neutral-800 font-body-md text-sm rounded-2xl rounded-br-sm border border-neutral-100 space-y-2 break-words whitespace-pre-wrap flex-1">{response}</div>
                  </div>
                )}
              </div>
            )}
